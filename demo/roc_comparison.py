...
def dca():
  ...
  for ax in axs:
    plt.sca(ax)
    dca_curve(y_0, y_hat_0, ticks=[1./101, 1./2], threshold_range=[1e-2,1-1e-2], fill_range = 0.25 if ax != axs[1] else None)
    dca_curve(y_1, y_hat_1, ticks=[1./101, 1./2], threshold_range=[1e-2,1-1e-2], fill_range = 0.15 if ax != axs[1] else None)
    if ax != axs[1]:
